...
def register(username: str, password: str) -> str:
    ...
    if len(password) < min_password_length:
        raise ValueError(f"Пароль должен содержать минимум {min_password_length} символа")
    ...
    return f"Пользователь '{username}' зарегистрирован (id={user_id}). Начальный баланс: {initial_balance:.2f} USD"
...
def buy(currency: str, amount: float) -> str:
    ...
    # Сохраняем старые балансы для логов
    ...
    return (
        f"Покупка выполнена: {amt:.4f} {code} ({currency_obj.name}) по курсу {rate:.4f} USD/{code}\n"
...
def sell(currency: str, amount: float) -> str:
    ...
    # Создаем USD кошелек если его нет
    if "USD" not in wallets_dict:
    ...
    return (
        f"Продажа выполнена: {amt:.4f} {code} ({currency_obj.name}) по курсу {rate:.4f} USD/{code}\n"
        f"Изменения в портфеле:\n"
...
def get_rate(cur_from: str, cur_to: str) -> str:
    ...
    if should_refresh_rates():
        # Здесь мог бы быть вызов API
        # Для демо просто используем кэш
        pass
    ...
    # Используем дефолтные курсы через USD
    default_rates = {
        "USD": 1.0,
        "EUR": 1.0786,
        "BTC": 59337.21,
...
def show_portfolio(base: str = "USD") -> str:
    ...
    for code, wallet in wallets.items():
        ...
        result += f"  Баланс: {wallet.balance:.4f} {code} → {value_in_base:.2f} {base}\n"
